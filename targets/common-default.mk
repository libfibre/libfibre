...
define parse_tgt
  ...
  $(eval $(4)S += $(filter-out $($(4)S),$(3)))
  ifeq (LIB,$(4))
    $(eval $(3)_name := lib$(3).a)
  else
    $(eval $(3)_name := $(3))
  endif
  $(eval $(3)_out := $(OUT_$(4))/$($(3)_name))
  $(eval $(3)_type := $(4))
  $(eval $(3)_make_dirs += $(filter-out $($(3)_make_dirs),$(1)))
  $(eval OUTS += $(filter-out $(OUTS),$($(3)_out)))
  $(foreach x,$(filter %.c,$($(3)_SOURCES)),$(eval $(call parse_source,$(1),$(2),$(3),$(x))))
  ifneq (none,$(myinstall))
    ifeq (,$(myinstall))
      $(3)_inst_path := $(INSTALL_$(4))
    else
      $(3)_inst_path := $(myinstall)
    endif
    $(3)_inst_flags := $(INSTALL_$(4)_FLAGS)
...
$($(1)_out): $(TOP_SRC)/$($(1)_dir)/$($(1)_src) $(TOP_SRC)/$($(1)_dir)/Makefile.am | $(OUT_OBJ)
	$$(Q)echo " [CC] $($(1)_title):$($(1)_dir)/$($(1)_src)"
	$$(Q)touch $($(1)_dep)
